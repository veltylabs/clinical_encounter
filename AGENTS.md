...
- **FSM constants live in `const.go`, out of scope for the harness migration.** `const.go` declares
  the visit lifecycle (`StatusCreated`, `StatusArrived`, `StatusTriaged`, `StatusInProgress`,
  `StatusCompleted`, `StatusCancelled`) — FHIR-aligned. No module event topics or transition table
  exist in this repo today; `docs/PLAN.md` adds `Topic*` constants in `model.go` per convention, but
  never edits `const.go` itself. Do not touch `const.go`.
- **The sibling `clinical_encounterOld` directory is legacy and out of scope — do not touch it,
  read it, or migrate it.** It has no importers in the rest of the ecosystem. This warning already
  existed in the retired `docs/PLAN_MODEL_MIGRATION.md` and carries forward verbatim here.
- **View reconciliation (resolved 2026-07-17):** an earlier plan (`docs/PLAN_UI_INITIAL_VIEW.md`,
  now retired/merged into `docs/PLAN.md`) built a WASM frontend directly inside this module —
  `web/client.go` importing `tinywasm/dom` + `tinywasm/layout/rightpanel` concretely, and a homemade
  `web/mockdb` package reimplementing `orm.Executor`/`orm.Compiler` as an in-memory store. Both
  violate this file's blacklist (a module may never import a concrete renderer, and never hand-rolls
  a port `storage/mem` already provides upstream) and are removed by `docs/PLAN.md`. The equivalent
  UI is rebuilt as `NewView(caller router.Caller) view.Presenter` at the module root, per the pattern
  in `veltylabs/item_catalog/view.go` — the app, not this module, will choose the concrete renderer
  (possibly still `tinywasm/layout`) that draws it. This discards in-browser-verified work (the old
  plan's "Estado de ejecución" checklist had 5 of 7 steps checked done, including a verified WASM
  build) — it is not a pure rename, see `docs/PLAN.md` Stage 0 for the full accounting.
